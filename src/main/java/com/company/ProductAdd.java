...
public class ProductAdd extends JFrame {
    ...
    JTextField codeenter = new JTextField();
    JTextField dostcenaenter = new JTextField();
    JTextField bezddsenter = new JTextField();

    String [] vidove = {"330 мл.", "500 мл."};
    JComboBox butilka = new JComboBox(vidove);

    JLabel addbtn = new JLabel(new ImageIcon("buttons/productaddbtn.png"));
        ...
        addbtn.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (markaenter.getText().isEmpty()) {
                    ...
                    if (FileActions.checkProductINS(marka, model)) {
                        JOptionPane.showMessageDialog(null, "Вече има съществуващ такъв продукт. Може да го промените чрез \"ПРОМЕНИ ПРОДУКТ\"");
                    } else {
                        String[] options = new String[2];
                        options[0] = new String("Потвърди");
                        options[1] = new String("Откажи");
                        int confirm = JOptionPane.showOptionDialog(addpanel.getContentPane(), "Потвърдете добавянето на продуктите", "Добавяне на продукт", 0, JOptionPane.INFORMATION_MESSAGE, null, options, null);
                        if (confirm == JOptionPane.YES_OPTION) {
                            addpanel.dispose();
                            String element = (String)butilka.getSelectedItem();
                            FileActions.insertProduct(marka, model+" ("+element+")", QTYenter.getText(), dostcenaenter.getText(), bezddsenter.getText(), codeenter.getText(), element);
                            JOptionPane.showMessageDialog(null, "Успешно добавихте продуктите.", "Успешно добавяне на продукт", JOptionPane.INFORMATION_MESSAGE);
                        }
                    }
                }
            }
